...
func (s *Service) Begin(ctx context.Context, actor identity.Actor, method, route, key string, bodyHash []byte) (Record, bool, error) {
	...
	now := s.clock.Now()
	record := Record{ActorID: actor.UserID, Method: method, Route: route, Key: key, Hash: idempotencyHashOwnership(bodyHash), Status: "processing", ExpiresAt: now.Add(s.ttl), UpdatedAt: now}
...
func (s *Service) Complete(ctx context.Context, record Record, code int, body []byte) error {
	if err := s.repository.Complete(ctx, record.ID, code, append([]byte(nil), body...), s.clock.Now()); err != nil {
