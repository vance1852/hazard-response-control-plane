...
func (s *Store) ActivateIncident(ctx context.Context, record hazard.ActivationRecord) (hazard.Incident, []hazard.Zone, error) {
	...
	err = s.WithinTx(ctx, func(tx *sql.Tx) error {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO outbox_events(id, topic, aggregate_type, aggregate_id, payload_json,
			status, attempts, available_at, created_at) VALUES(?, ?, 'incident', ?, ?, 'pending', 0, ?, ?)`, outboxID,
			record.Topic, incident.ID, record.Payload, formatTime(record.Now), formatTime(record.Now))
		if err != nil {
