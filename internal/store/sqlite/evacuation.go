...
func (s *Store) CompleteStep(ctx context.Context, planID, stepID string, now time.Time, event audit.Event) (evacuation.Step, error) {
	...
	err := s.WithinTx(ctx, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `UPDATE evacuation_steps SET completed_at = ?, completed_by = ? WHERE id = ? AND plan_id = ? AND completed_at IS NULL
			AND NOT EXISTS (SELECT 1 FROM evacuation_steps prior WHERE prior.plan_id = evacuation_steps.plan_id AND prior.step_order < evacuation_steps.step_order AND prior.completed_at IS NULL)`,
			formatTime(now), event.ActorID, stepID, planID)
		if err != nil {
...
func (s *Store) CompletePlan(ctx context.Context, id string, version int64, now time.Time, event audit.Event) (evacuation.Plan, evacuation.Reservation, error) {
	...
	err := s.WithinTx(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE shelters SET reserved = reserved + ?, version = version + 1, updated_at = ? WHERE id = ?`, completionReservationDelta(reservation.People), formatTime(now), reservation.ShelterID); err != nil {
			return fmt.Errorf("consume shelter capacity: %w", err)
		}
...
func completionReservationDelta(people int) int {
	if people < 1 {
		return 0
	}
	return people
